	...
	if(isset($_POST["submitPic"])) {
		...
		if(!empty($_FILES["fileToUpload"]["name"])){
			...
			// kui kõik korras, laeme üles
				...
				$waterMarkWith = imagesx($waterMark);
				$waterMarkHeight = imagesy($waterMark);
				$waterMarkPosX = $newWidth - $waterMarkWith - 10;
				$waterMarkPosY = $newHeight - $waterMarkHeight - 10;
				//kopeerin vesimärgi pikslid pildile
				imagecopy($myImage, $waterMark, $waterMarkPosX, $waterMarkPosY, 0, 0, $waterMarkWith, $waterMarkHeight);
				
				//Lisame ka teksti
				$textToImage = "Veebiprogrammeerimine";
				$textColor = imagecolorallocatealpha($myImage, 255, 255, 255, 60);
				//alpha 0...127
				imagettftext($myImage, 20, -43, 10, 30, $textColor, "../vp_picfiles/ARIALBD.TTF", $textToImage);
